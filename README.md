...
- the app's `mount(context)` renders the UI. Its context is a `SurfaceViewContext`: that same flat `SurfaceContext`, plus the container, the warm-keep `lifecycle` (focus and blur signals — a blurred view stays mounted and warm), and the host-chrome verbs. (Mount contexts are named `{Realm}{UseCase}Context` — the name says which realm mounted you and as what.)
