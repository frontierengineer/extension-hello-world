...
- Live worker heartbeats pushed straight from each connected machine over the platform channel
...
**All three realms read the same way.** The surface model above — `register()` names components, logic lives in each component's `mount()`, and `mount()` returns the teardown — is the shape of the **host** and **worker** realms too, so read `host/index.ts` and `worker/index.ts` next to `surface/index.tsx` and they mirror each other. Each realm's `register()` is declaration-only and names top-level components: the host realm takes a single daemon (`h.daemon.register(...)`), and the worker realm takes up to three kinds — an agent **runtime** (`w.runtime.register`), **workspaces** (`w.workspace.register`), and the general-purpose **daemon** (`w.daemon.register`), exactly parallel to the surface's application/sidebar/daemon (Hello World's worker contributes only a daemon). Every component's `mount()` receives a **flat**, `{Realm}{UseCase}Context`-named context with every capability directly on it (`context.store`, `context.scheduler`, `context.channel`, `context.mcp` on the host daemon; `context.channel`, `context.execute`, `context.modules` on the worker daemon — no `.services.` hop) and returns the same optional `dispose`. There is no separate top-level unload hook in any realm: Hello World's worker daemon returns a `dispose` that clears its heartbeat interval, and its host daemon returns an empty handle because everything it made is platform-tracked — the single teardown path each realm has, captured in `mount` and closed over by `dispose`. The worker link is a **platform channel** any of the extension's surface or host code addresses directly with a target — `{ machine }` for machine-scoped work, `{ reservationId }` for slot-scoped work (the platform resolves the reservation to its machine and stamps it on the delivery envelope). It speaks two dialects: `channel.request(target, …)` for ask-and-await (the platform owns the correlation and the timeout; the worker answers with `channel.onRequest`) and `send()`/`onMessage()` for fire-and-forget pushes like the heartbeat — which every subscriber receives directly, with no host-bundle relay.
...
"realms": {
  ...
  "worker":  { "runtimes": [], "workspaces": [], "daemon": true }
}
```
