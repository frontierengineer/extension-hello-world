...
//    §5  Private bus    — request/respond + publish for THIS application's UI
//    §6  Public bus     — one versioned endpoint OTHER applications can call
...
//     the host writes a types shim ABOVE the application dir (extensions/types.ts),
//     a sibling of every installed application. (Standalone: the vendored copy at
//     this repo's root, reached the same way — see README.)
//   • `../messages`   — THIS application's OWN shared file at its root, one level
...
// ═══════════════════════════════════════════════════════════════════════════
//  WHY this exists: the host NEVER understands an application's Store — it's raw
...
//    • application.json declares `dataVersion: N` — the format THIS package ships.
...
//      the application unloaded, so the next reload retries — never half-migrated.
//
...
export function register(serverProvider: ServerProvider): void {
  ...
  // `bus` is this application's bus; `workers` reaches its worker component;
  ...
  // ─────────────────────────────────────────────────────────────────────────
  //  §1  STORE — durable, per-application key/value state
  // ─────────────────────────────────────────────────────────────────────────
  //  TIER: server owns persistence. The Store is private to this application
  //  (its own dir on disk) and survives restarts AND application updates. The UI
  ...
  async function writeState(next: HelloState): Promise<HelloState> {
    ...
    bus.application.publish('state.changed', next);
    return next;
  }
  ...
  //  sets it; the application only READS. Declaring a field is all it takes for
  //  it to appear in the host's auto-generated settings UI. `config.watch`
  //  fires when the user saves — the application reacts live (no reload).
  config.declare({
    key: 'greeting',
    label: 'Greeting',
  ...
  //  §4  SCHEDULER — a recurring, host-managed timer
  // ─────────────────────────────────────────────────────────────────────────
  //  TIER: server. Instead of running its own setInterval, the application hands
  ...
  scheduler.register({
    ...
    handler: async () => {
      ...
      bus.application.publish('state.changed', state);
    },
  });
  ...
  //  §5  PRIVATE BUS — request/respond + publish for THIS application's UI
  // ─────────────────────────────────────────────────────────────────────────
  //  TIER: server answers its own UI. `bus.application` is fully PRIVATE — no
  //  other application can see it. A `request` from ui/index.tsx lands on the
  ...
  //  subscribed. This is the application's frontend↔backend microservice link.
  //
  ...
  bus.application.respond('state.get', async () => {
    await ready;
    return readState();
  });

  bus.application.respond('state.bump', async (params: { by?: number }) => {
    await ready;
    const state = await readState();
  ...
  bus.application.respond('note.set', async (params: { note: string }) => {
    await ready;
    const state = await readState();
  ...
  //  §6  PUBLIC BUS — one versioned endpoint OTHER applications can call
  // ─────────────────────────────────────────────────────────────────────────
  //  TIER: server, cross-application contract. `bus.public.respond(topic,
  //  version, handler)` exposes ONE endpoint outside this application. Another
  //  application reaches it read-only via `bus.applications('hello-world')
  ...
  //  link to THIS application's worker component on a connected machine. The
  ...
  function channelFor(machine: string): WorkerChannel {
    ...
    ch.onMessage((raw: any) => {
      ...
      if (msg.kind === 'inspect.res') {
        ...
        bus.application.publish('worker.heartbeat', {
          machine,
  ...
  bus.application.respond('worker.inspect', async (params: { machine: string }) => {
